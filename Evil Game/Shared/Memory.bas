...
Function GetAddress(ByVal Address As Long) As Long 'For use with the AddressOf operator
    'On Error Resume Next
    
    GetAddress = Address
End Function

'Of course there is no Floor function...since Fix does this already
Function Ceiling(Value As Double) As Long
    'On Error Resume Next
    
    If Value - Fix(Value) > 0 Then
        Ceiling = CLng(Value) + 1
    ElseIf Value - Fix(Value) < 0 Then
        Ceiling = CLng(Value) - 1
    Else
        Ceiling = CLng(Value)
    End If
...
'These should be close to the fastest functions in VB you can write for these
...
Function IsEven(Value As Long) As Boolean
    ...
    IsEven = (Value And 1) = 0
End Function

Sub SetFlag(Value As Long, Flag As Long, Toggle As Boolean)
    'On Error Resume Next
    
    Value = Value And Not Flag Or IIf(Toggle, Flag, 0&)
End Sub

Function GetFlag(Value As Long, Flag As Long) As Boolean
    'On Error Resume Next
    
    GetFlag = (Value And Flag) = Flag
End Function

Function FixHex(HexString As String, Width As Byte) As String
    'On Error Resume Next
    
    If Width > Len(HexString) Then
        FixHex = String$(Width - Len(HexString), "0") & HexString
    Else
        FixHex = HexString
    End If
End Function
